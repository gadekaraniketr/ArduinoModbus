...
static ssize_t _modbus_rtu_recv(modbus_t *ctx, uint8_t *rsp, int rsp_length)
{
    ...
    return ((modbus_rtu_t*)ctx->backend_data)->port->readBytes(rsp, rsp_length);
#else
...
#elif defined(ARDUINO)
    ctx_rtu->port->begin(ctx_rtu->baud, ctx_rtu->config);
    ctx_rtu->port->receive();
#else
...
static int _modbus_rtu_flush(modbus_t *ctx)
{
	...
	while (((modbus_rtu_t*)ctx->backend_data)->port->available()) {
        ((modbus_rtu_t*)ctx->backend_data)->port->read();
    }
                              ...
                              struct timeval *tv, int length_to_read)
{
    ...
    do {
        ...
        s_rc = ((modbus_rtu_t*)ctx->backend_data)->port->available();
...
#ifdef ARDUINO
modbus_t* modbus_new_rtu(int port, unsigned long baud, uint16_t config)
#else
...
#endif
{
	...
	if(port == 0)
		ctx_rtu->port = new RS485Class(SERIAL_PORT_HARDWARE, 1, 2, 3);
